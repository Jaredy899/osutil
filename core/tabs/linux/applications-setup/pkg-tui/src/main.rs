...
fn run_fzf(
    ...
    let header = format!("{} | Enter/Alt-i: Install | Alt-r: Remove", manager.name());
...
fn action_command(manager: PackageManager, action: Action) -> (&'static str, Vec<String>, bool) {
    match (manager, action) {
        ...
        (PackageManager::Dnf, Action::Remove) => ("dnf", vec!["remove".into(), "-y".into()], true),
        (PackageManager::RpmOstree, Action::Install) => {
            ("rpm-ostree", vec!["install".into()], true)
        }
        ...
        (PackageManager::Moss, Action::Install) => {
            ("moss", vec!["install".into(), "-y".into()], false)
        }
        (PackageManager::Moss, Action::Remove) => {
            ("moss", vec!["remove".into(), "-y".into()], false)
        }
        (PackageManager::XbpsInstall, Action::Install) => ("xbps-install", vec!["-y".into()], true),
        (PackageManager::XbpsInstall, Action::Remove) => ("xbps-remove", vec!["-y".into()], true),
        (PackageManager::Pkg, Action::Install) => {
            ("pkg", vec!["install".into(), "-y".into()], true)
        }
        (PackageManager::Pkg, Action::Remove) => ("pkg", vec!["remove".into(), "-y".into()], true),
        (PackageManager::Brew, Action::Install) => {
            ("brew", vec!["install".into(), "--formula".into()], false)
        }
        (PackageManager::Brew, Action::Remove) => {
            ("brew", vec!["uninstall".into(), "--formula".into()], false)
        }
    }
}
...
fn fetch_package_list(manager: PackageManager, kind: CacheKind) -> Result<Vec<String>, String> {
    let raw = match (manager, kind) {
        (PackageManager::Nala, CacheKind::Available)
        | (PackageManager::Apt, CacheKind::Available) => run_capture("apt-cache", &["pkgnames"])?,
        ...
        (PackageManager::Zypper, CacheKind::Available) => run_shell_capture(
            "zypper se -s | awk 'NR>2 {print $2; print $3}' | grep -v '^[|]' | sort -u",
        )?,
        ...
        (PackageManager::Moss, CacheKind::Available) => {
            run_capture("moss", &["list", "available"])?
        }
        (PackageManager::XbpsInstall, CacheKind::Available) => {
            run_shell_capture("xbps-query -Rs '' | awk '{print $2}'")?
        }
        (PackageManager::Pkg, CacheKind::Available) => {
            run_shell_capture("pkg search . | awk '{print $1}'")?
        }
        (PackageManager::Brew, CacheKind::Available) => run_capture("brew", &["formulae"])?,

        (PackageManager::Nala, CacheKind::Installed)
        | (PackageManager::Apt, CacheKind::Installed) => {
        ...
        (PackageManager::Moss, CacheKind::Installed) => {
            run_capture("moss", &["list", "installed"])?
        }
