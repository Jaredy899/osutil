...
installNeovim() {
    ...
    case "$PACKAGER" in
        pacman)
            installPkg neovim git fzf ripgrep fd tree-sitter gcc || true
            ;;
        apt-get|nala)
            installPkg git ripgrep fd-find tree-sitter-cli gcc || true # fzf will be installed from git
            ...
            ;;
        dnf|rpm-ostree)
            installPkg neovim git fzf ripgrep fd-find tree-sitter-cli gcc || true
            ;;
        zypper)
            installPkg neovim git fzf ripgrep fd tree-sitter gcc || true
            ;;
        apk|xbps-install|eopkg|moss|pkg)
            installPkg neovim git fzf ripgrep fd tree-sitter gcc || true
            ;;
        *)
            unsupportedPackager
